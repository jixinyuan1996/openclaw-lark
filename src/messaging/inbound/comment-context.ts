...
function buildDriveCommentSurfacePrompt(params: {
  ...
  lines.push(
    ...
    'If you need to inspect or handle the comment thread, prefer the weact_drive tools: use list_comments / list_comment_replies to inspect comments, and use reply_comment/add_comment to notify the user after modifying the document.',
    'If the comment asks you to modify document content, such as adding, inserting, replacing, or deleting text, tables, or headings, you must first use weact_doc to actually modify the document. Do not reply with only "done", "I\'ll handle it", or a restated plan without calling tools.',
    'If the comment quotes document content, that quoted text is usually the edit anchor. For requests like "insert xxx below this content", first locate the position around the quoted content, then use weact_doc to make the change.',
    ...
    'For requests like "summarize the content below", "explain this section", or "continue writing from here", first locate the relevant document fragment based on the comment\'s quoted content. If the quote is not sufficient to support the answer, then use weact_doc.read or weact_doc.list_blocks to read nearby context.',
    ...
    'When document edits are involved, first use weact_doc.read or weact_doc.list_blocks to confirm the context, then use weact_doc writing or updating capabilities to complete the change. After the edit succeeds, notify the user through weact_drive.reply_comment.',
    ...
    'If you have already completed the user-visible action through weact_drive.reply_comment or weact_drive.add_comment, output NO_REPLY at the end to avoid duplicate sending.',
