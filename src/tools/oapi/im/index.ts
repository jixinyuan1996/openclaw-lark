...
export function registerFeishuImTools(api: OpenClawPluginApi): void {
  ...
  if (registerFeishuImUserMessageTool(api)) registered.push('weact_im_user_message');
  if (registerFeishuImUserFetchResourceTool(api)) registered.push('weact_im_user_fetch_resource');
  registered.push(...registerMessageReadTools(api));
  if (registered.length > 0) {
    api.logger.debug?.(`weact_im: Registered ${registered.join(', ')}`);
  }
}
